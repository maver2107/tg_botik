...
from src.bot.models.gender import Gender  # Импортируем наш ENUM
...
class QuestionnaireService:
    ...
    async def process_age(self, age_text: str, state: FSMContext) -> dict:
        ...
        try:
            ...
            if age < 10 or age > 100:
                ...
                return {"success": False, "message": "Пожалуйста, введите реальный возраст (10-100):"}

            await state.update_data(age=age)
            await state.set_state(FormStates.waiting_for_gender)
            logger.debug(f"Возраст {age} принят")
            return {"success": True, "message": "age_processed"}

        except ValueError:
            logger.warning(f"Нечисловой возраст: {age_text}")
            return {"success": False, "message": "Пожалуйста, введите число:"}

    async def process_gender(self, gender_text: str, state: FSMContext) -> dict:
        ...
        if gender_text == "👨 Мужской":
            ...
            return {"success": False, "message": "Пожалуйста, выбери пол из предложенных вариантов:"}
        ...
        return {"success": True, "message": "gender_processed"}
    ...
    async def complete_questionnaire(self, interests: str, user_id: int, state: FSMContext) -> str:
        ...
        # Сохраняем в базу (ENUM автоматически преобразуется в строку)
        await self.users_dao.update_user_data(
            tg_id=user_id,
            name=form_data.get("name"),
            age=form_data.get("age"),
            user_gender=form_data.get("user_gender"),  # Здесь будет Gender.MALE или Gender.FEMALE
